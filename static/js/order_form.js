...
const need_gift = (sum) => sum >= Consts.SUM_FOR_GIFT
...
const post_discount = ({
        ...
        packet,
        sum = 0
    }) => (sum < Consts.SUM_FOR_POST_DISCOUNT) ? 0 :
    (post_cost_with_packet({
        post_cost,
        packet
    }) * parseInt(Consts.POST_DISCOUNT_PERCENT) / 100).toFixed(2)
...
const total_sum = ({
    ...
    packet,
    sum
}) => parseInt(Number(sum) + total_postals({
    post_cost,
    packet,
    sum
}))
...
var set_sum_totals = ({
    ...
    packet = Number($(tags.packet.packet).val()),
