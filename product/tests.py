...
class ProductAPITestCase(APITestCase):
    ...
    def test_retrieve_product(self):
        ...
        """
        url = reverse('api:product-detail', args=[self.product2.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.data
    ...
    def test_destroy_product(self):
        ...
        """
        url = reverse('api:product-detail',
                      args=[Product.objects.latest('id').id])
        products_count_before = Product.objects.count()
    ...
    def test_search_products(self):
        ...
        """
        url = reverse('api:product-list') + "?term=" + self.product2.name
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
